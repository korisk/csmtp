...
Command_Entry* FindCommandEntry(SMTP_COMMAND command)
{
	...
	for(unsigned int i = 0; i < sizeof(command_list)/sizeof(command_list[0]); ++i)
	{
...
void CSmtp::FormatHeader(char* header)
{
	...
	string str =  "=?UTF-8?B?";
	str += base64_encode((unsigned char const*)(m_sSubject.c_str())  , m_sSubject.size());
	str += "?=";
...
//	  strcat(header, m_sSubject.c_str());
	...
	if(!Attachments.size())
	{ // no attachments
		if(m_bHTML) strcat(header,"Content-Type: text/html; charset=\"UTF-8\"\r\n");
		else strcat(header,"Content-type: text/plain; charset=\"UTF-8\"\r\n");
	...
	{ // there is one or more attachments
		...
		strcat(SendBuf,"\r\n");
		if(m_bHTML) strcat(SendBuf,"Content-type: text/html; charset=UTF-8\r\n");
		else strcat(SendBuf,"Content-type: text/plain; charset=UTF-8\r\n");
...
{
	std::cout << "cleanup " << m_ssl << " " << m_ctx << std::endl;
	if(m_ssl != NULL)
	{
